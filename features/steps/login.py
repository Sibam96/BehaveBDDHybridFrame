...
@when(u'I enter valid email address as "{email}" and valid passwords as "{password}" into the fields')
...
@when(u'I enter valid email say "{email}" and invalid passwords say "{password}" into the fields')
def step_impl(context,email,password):
    context.login_page.enter_email_address(email)
    context.login_page.enter_password(password)
...
@when(u'I enter invalid email and invalid passwords say "{password}" into the fields')
def step_impl(context,password):
    invalid_email = EmailWithTimeStampGenerator.get_new_email_with_timestamp()
    context.login_page.enter_email_address(invalid_email)
    context.login_page.enter_password(password)
